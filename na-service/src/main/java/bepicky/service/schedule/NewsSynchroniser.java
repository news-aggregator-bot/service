...
import static bepicky.service.entity.NewsNoteNotificationEntity.Link.TAG;
...
public class NewsSynchroniser {
    ...
    public void sync() {
        ...
        Set<NewsNoteEntity> actualNotes = latestNewsNoteId != 0 ?
        ...
        actualNotes.stream()
            .collect(Collectors.groupingBy(NewsNoteEntity::getSourcePages, Collectors.toSet()))
        ...
        latestNewsNoteId = actualNotes.stream()
            .mapToLong(NewsNoteEntity::getId)
            .max()
            .orElseGet(() -> latestNewsNoteId);
    }

    private Stream<ReaderEntity> unfoldSourcePages(Collection<SourcePageEntity> sps) {
        return sps.stream().flatMap(this::findApplicableReaders);
    }

    private Stream<ReaderEntity> findApplicableReaders(SourcePageEntity sp) {
    ...
    private Stream<ReaderEntity> filterReaders(SourcePageEntity sp, Collection<CategoryEntity> categories) {
        return categories
            .stream()
            .map(CategoryEntity::getReaders)
            .flatMap(Set::stream)
