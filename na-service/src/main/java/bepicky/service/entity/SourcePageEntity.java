...
public class SourcePageEntity extends DatedEntity {
    ...
    @Transient
    private Multimap<CategoryType, CategoryEntity> typedCategories;

    public Collection<CategoryEntity> getRegions() {
        initTypedCategories();
        return typedCategories.get(CategoryType.REGION);
    }

    public Collection<CategoryEntity> getCommon() {
        initTypedCategories();
    ...
    private void initTypedCategories() {
        if (typedCategories == null || typedCategories.isEmpty()) {
            typedCategories = Multimaps.index(categories, CategoryEntity::getType);
        }
    }
}
